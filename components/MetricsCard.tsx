...
import { LedgerType, Transaction, Property, BudgetCategory } from '../types';
...
export const MetricsCard: React.FC<MetricsCardProps> = ({ mode, transactions, properties }) => {
  const { calculatedBudgetsByLedger } = useData();
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      let value = 0;
      if (mode === 'active') {
        value = monthTx.reduce((acc, t) => (t.amount || 0) > 0 ? acc + (t.amount || 0) : acc, 0);
      } else if (mode === 'passive') {
        value = monthTx.reduce((acc, t) => acc + (t.amount || 0), 0);
      } else {
        value = monthTx.reduce((acc, t) => (t.amount || 0) < 0 ? acc + Math.abs(t.amount || 0) : acc, 0);
      }

      data.push({ month: monthLabel, value: value || 0 });
    }
    return data;
  ...
  if (mode === 'active') {
    ...
    const gci = modeTransactions.reduce((acc, t) => (t.amount || 0) > 0 ? acc + (t.amount || 0) : acc, 0);
    const estTax = (gci || 0) * 0.2; 

    label1 = 'YTD GCI'; 
    val1 = `$${((gci || 0) / 1000).toFixed(1)}k`;
    label2 = 'Est. Tax (T2125)'; 
    val2 = `$${((estTax || 0) / 1000).toFixed(1)}k`;
    sub2 = 'Approx. 20%';

  } else if (mode === 'passive') {
    color = '#06b6d4'; // cyan-500
    const noi = modeTransactions.reduce((acc, t) => acc + (t.amount || 0), 0);
    const cca = (properties || []).reduce((sum, p) => sum + ((p.openingUcc || 0) * 0.04), 0);

    label1 = 'YTD Net Op Income'; 
    val1 = `$${((noi || 0) / 1000).toFixed(1)}k`;
    label2 = 'Est. Annual CCA'; 
    val2 = `$${((cca || 0) / 1000).toFixed(1)}k`;
    sub2 = 'Based on UCC';
    ...
    const personalBudgets = calculatedBudgetsByLedger?.personal || [];
    const totalSpentInBudgets = personalBudgets.reduce((sum, b) => sum + (b.spent || 0), 0);
    const totalAllotted = personalBudgets.reduce((sum, b) => sum + (b.limit || 0), 0);
    const budgetUtil = totalAllotted > 0 ? Math.round((totalSpentInBudgets / totalAllotted) * 100) : 0;

    label1 = 'Monthly Spend'; 
    val1 = `$${(monthlySpend || 0).toLocaleString()}`;
    label2 = 'Savings Rate'; 
    val2 = `${Math.max(0, savingsRate)}%`;
    sub2 = `Budget Utilized: ${budgetUtil || 0}%`;
  }
