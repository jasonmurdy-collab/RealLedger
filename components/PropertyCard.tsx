...
export const PropertyCard: React.FC<PropertyCardProps> = ({ property, transactions }) => {
  ...
  const noi = propertyTransactions.reduce((acc, t) => acc + (t.amount || 0), 0); 
  const currentVal = property?.currentValue || 0;
  const capRate = currentVal > 0 ? (noi / currentVal) * 100 : 0;
  
  const ccaRate = 0.04; // Class 1
  const maxCca = ((property?.openingUcc || 0) + ((property?.additions || 0) * 0.5)) * ccaRate;
    ...
    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/5 rounded-2xl p-4 mb-4 relative overflow-hidden group hover:border-cyan-500/30 transition-colors shadow-sm">
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-zinc-900 dark:text-white font-semibold text-sm">{property?.address || 'Unnamed Property'}</h3>
            <p className="text-zinc-500 text-xs">Class {property?.ccaClass || 1} Asset</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 py-3 border-t border-zinc-100 dark:border-white/5">
        <div>
          ...
          <p className="text-zinc-900 dark:text-white font-bold">${((currentVal || 0) / 1000).toFixed(0)}k</p>
        </div>
        <div>
          <p className="text-zinc-400 text-[10px] uppercase tracking-wider mb-1">Max CCA Claim</p>
          <p className="text-cyan-600 dark:text-cyan-400 font-bold">${(maxCca || 0).toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 mt-2">
         <div className="flex items-center gap-1.5 text-xs text-zinc-500 dark:text-zinc-400">
            <TrendingUp size={12} />
            <span>Cap: {(capRate || 0).toFixed(1)}%</span>
         </div>
         <div className="flex items-center gap-1.5 text-xs text-zinc-500 dark:text-zinc-400">
            <Users size={12} />
