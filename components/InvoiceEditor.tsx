...
import { Invoice, LineItem, UserProfile } from '../types';
import { X, Save, Trash2, Plus, FileText, Send, CheckCircle, Download } from 'lucide-react';
...
export const InvoiceEditor: React.FC<InvoiceEditorProps> = ({ isOpen, onClose, onSave, onDelete, invoice, profile }) => {
    ...
    <>
      {isGeneratingPdf && (
        <div style={{ position: 'absolute', left: '-9999px', top: 0 }}>
          <InvoicePDF ref={pdfRef} invoice={formData} profile={profile} />
        </div>
      )}
      <div className="fixed inset-0 z-50 bg-zinc-950 p-4 sm:p-6 animate-slide-up">
        <div className="max-w-4xl mx-auto h-full flex flex-col">
          ...
          <footer className="flex-shrink-0 flex flex-wrap justify-between items-center gap-4 py-4 border-t border-white/10">
            <div>
              <button onClick={handleGeneratePdf} disabled={isGeneratingPdf} className="px-4 py-2 text-rose-400 text-sm font-semibold flex items-center gap-2 bg-rose-500/10 rounded-lg hover:bg-rose-500/20 disabled:opacity-50">
