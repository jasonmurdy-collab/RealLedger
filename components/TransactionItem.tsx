        ...
        <div className="flex items-center gap-3 p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800/50 rounded-lg group transition-colors">
            ...
            <div className="flex-1">
                <div className="flex items-center gap-2">
                    <p className="font-semibold text-sm text-zinc-900 dark:text-white truncate max-w-[120px]">{tx?.vendor || 'Unknown'}</p>
            ...
            <div className="text-right">
                ...
                <p className="text-[10px] text-zinc-400 uppercase">
                    {tx?.date ? new Date(tx.date).toLocaleDateString('en-CA', { month: 'short', day: 'numeric' }) : '-'}
                </p>
            </div>
        </div>
    );
